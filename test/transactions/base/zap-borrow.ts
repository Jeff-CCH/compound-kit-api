...
describe('Transaction: Zap Borrow', function () {
  ...
  it('user zap borrow USDbC in USDbC market', async function () {
    // 1. user obtains a quotation for zap borrow 100 USDbC through the zap borrow API
    ...
    // 4. user's USDbC borrow balance should increase.
    ...
    // TODO: undefined nativeToken error
    const balance = await service.getBalance(user.address, destToken);
    console.log('check if the latter 2 numbers are close');
    console.log(balance.amount);
    console.log(quotation.quotation.destAmount);
  });

  it('user zap borrow USDC in USDbC market', async function () {
    // 1. user obtains a quotation for zap borrow USDC from 100 USDbC through the zap borrow API
