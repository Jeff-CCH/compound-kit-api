...
describe('Transaction: Leverage', function () {
  ...
  before(async function () {
    ...
    await claimToken(chainId, user.address, baseTokens.WETH, '5');
  });

  snapshotAndRevertOnce();

  it('user leverage his USDbC market position with WETH', async function () {
    const marketId = logics.compoundv3.MarketId.USDbC;

    // 1. user has supplied 5 WETH
    const supplyAmount = new common.TokenAmount(baseTokens.WETH, '5');
    await utils.supply(chainId, user, marketId, supplyAmount);

    // 2. user has borrowed 2000 USDbC
    const borrowAmount = new common.TokenAmount(baseTokens.USDbC, '2000');
    ...
    const leverageAmount = new common.TokenAmount(baseTokens.WETH, '3');
    const slippage = 100;
    ...
    console.log(quotation.approvals);
    ...
    process.stdout.write(JSON.stringify(transactionRequest) + '\n');
